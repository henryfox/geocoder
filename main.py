...
class replyhandler(Handler):
	def get(self):
		...
		info = urlopen("https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=%s" % (self.adress, api_key))
		info = info.read()
		j = json.loads(info)
		geometry = j["results"][0]["geometry"]["location"]
		lat = geometry["lat"]
		lng = geometry["lng"]
		self.render("response.html", title="response", lat=lat, lng=lng)
